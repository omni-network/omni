...
/**
 * @title NominaPortal_exec_Test
 * @dev Test of NominaPortal._exec, an internal function made public for testing
 */
contract NominaPortal_exec_Test is Base {
    ...
    function test_syscall_forwardsRevert() public {
        // We use NominaPortal.addValidatorSet, as that is a valid system call

        XTypes.Validator[] memory emptyValSet; // doesn't matter
        uint64 valSetId = 3; // doesn't matter
        bytes memory data = abi.encodeWithSelector(NominaPortal.addValidatorSet.selector, valSetId, emptyValSet);

        // this will revert with "only cchain" because _xmsg.sourceChainId won't be set
        vm.expectRevert("NominaPortal: only cchain");
        portal.syscall(data);
    }
