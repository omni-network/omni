...
import { OAppCore, OAppReceiver, Origin } from "./ext/layerzero/oapp/OAppReceiver.sol";
import { IERC7683 } from "./erc7683/IERC7683.sol";
import { ISolverNetInboxLZ } from "./interfaces/ISolverNetInboxLZ.sol";
...
/**
 * @title SolverNetInboxLZ
 * @dev This version is capable of receiving messages from non-Core chains via LayerZero in addition to Omni Core.
 * @notice Entrypoint and alt-mempool for user solve orders.
 */
contract SolverNetInboxLZ is
    OwnableRoles,
    ReentrancyGuard,
    Initializable,
    DeployedAt,
    ...
    ISolverNetInboxLZ
{
    ...
    /**
     * @notice Fill an order via LayerZero.
     * @dev Only callable by the outbox.
    ...
    function _lzReceive(Origin calldata _origin, bytes32, bytes calldata _message, address, bytes calldata)
        ...
        override
    {
        (bytes32 id, bytes32 fillHash, address creditedTo) = abi.decode(_message, (bytes32, bytes32, address));
        _markFilled(id, fillHash, creditedTo, _origin.srcEid, _origin.sender.toAddress());
    }
    ...
    function _getPeer(uint32 _eid) internal view override returns (bytes32) {
        return _outboxes[_eid].toBytes32();
    }
    ...
    function _getPeerOrRevert(uint32 _eid) internal view override returns (bytes32) {
        ...
        if (peer == bytes32(0)) revert InvalidOutbox();
        return peer;
    }
}
