...
interface ISolverNet {
    ...
    /**
     * @notice Token expense required to fulfill an order.
     * @param token    Token address, 0x0 for native.
     ...
     * @param data         Calldata.
     * @param expenses     Expenses required to fulfill the call.
     */
    struct Call {
        uint64 destChainId;
        bytes32 target;
     ...
     * @param deposits  Deposits payed by user, locked on source chain. Awarded to solver on fulfillment.
     * @param expenses  Expenses required to fulfill the order on the destination chain, payed by solver.
     */
    struct OrderData {
        Call call;
        Deposit[] deposits;
