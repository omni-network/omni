...
/**
 * @title XTypes
 * @dev Defines xchain types, core to Nomina's xchain messaging protocol. These
...
library XTypes {
     ...
     *        throughout Nomina's cross-chain messaging protocol. Msg is used to construct and verify
     ...
     * @custom:field sourceChainId      Chain ID of the source chain
     * @custom:field consensusChainId   Chain ID of the Nomina consensus chain
    ...
    /**
     * @notice The required parameters to submit xmsgs to a NominaPortal. Constructed by the relayer
     *         by watching Nomina's consensus chain, and source chain blocks.
    ...
    /**
     * @notice A Nomina validator, specified by their ethereum address and voting power.
    ...
    /**
     * @notice A chain in the "nomina network" specified by its chain ID and supported shards.
