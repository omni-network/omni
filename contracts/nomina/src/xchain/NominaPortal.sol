...
import { INominaPortalSys } from "src/interfaces/INominaPortalSys.sol";
import { INominaPortalAdmin } from "src/interfaces/INominaPortalAdmin.sol";
...
import { NominaPortalConstants } from "./NominaPortalConstants.sol";
import { NominaPortalStorage } from "./NominaPortalStorage.sol";

contract NominaPortal is
    INominaPortal,
    INominaPortalSys,
    INominaPortalAdmin,
    OwnableUpgradeable,
    ...
    NominaPortalStorage
{
     ...
     *         Available actions are ActionXCall and ActionXSubmit, defined in NominaPortalConstants.sol.
     */
    modifier whenNotPaused(bytes32 actionId, uint64 chainId_) {
        require(!_isPaused(actionId, _chainActionId(actionId, chainId_)), "NominaPortal: paused");
        _;
    }

    /**
     * @notice Construct the NominaPortal contract
     */
    constructor() {
     ...
     * @custom:field nominaChainId          Chain ID of Nomina's EVM execution chain
     * @custom:field nominaCChainId         Chain ID of Nomina's consensus chain
    ...
    struct InitParams {
        address owner;
        address feeOracle;
        uint64 nominaChainId;
        uint64 nominaCChainId;
    ...
    /**
     * @notice Initialize the NominaPortal contract
     */
    function initialize(InitParams calldata p) public initializer {
        ...
        nominaChainId = p.nominaChainId;
        nominaCChainId = p.nominaCChainId;

        // nomina consensus chain uses Finalized+Broadcast shard
        uint64 nominaCShard = ConfLevel.toBroadcastShard(ConfLevel.Finalized);
        _setInXMsgOffset(p.nominaCChainId, nominaCShard, p.cChainXMsgOffset);
        _setInXBlockOffset(p.nominaCChainId, nominaCShard, p.cChainXBlockOffset);
    }
    ...
    /**
     * @dev View function replaces original variable name for backwards compatibility
     */
    function omniChainId() public view returns (uint64) {
        return nominaChainId;
    }

    /**
     * @dev View function replaces original variable name for backwards compatibility
     */
    function omniCChainId() public view returns (uint64) {
        return nominaCChainId;
    }
    ...
    {
        require(isSupportedDest[destChainId], "NominaPortal: unsupported dest");
        ...
        require(data.length <= xmsgMaxDataSize, "NominaPortal: data too large");

        // conf level will always be last byte of shardId. for now, shardId is just conf level
        uint64 shardId = uint64(conf);
        require(isSupportedShard[shardId], "NominaPortal: unsupported shard");
        ...
        nonReentrant
    {
        ...
        require(xheader.consensusChainId == nominaCChainId, "NominaPortal: wrong cchain ID");
        require(xmsgs.length > 0, "NominaPortal: no xmsgs");
        require(valSetTotalPower[valSetId] > 0, "NominaPortal: unknown val set");
        require(valSetId >= _minValSet(), "NominaPortal: old val set");
    ...
    function _exec(XTypes.BlockHeader calldata xheader, XTypes.Msg calldata xmsg_) internal {
        ...
        require(destChainId == chainId() || destChainId == BroadcastChainId, "NominaPortal: wrong dest chain");
        require(offset == inXMsgOffset[sourceChainId][shardId] + 1, "NominaPortal: wrong offset");
        ...
        if (xmsg_.to == address(this)) {
            emit XReceipt(
                ...
                msg.sender,
                false,
                abi.encodeWithSignature("Error(string)", "NominaPortal: no xcall to portal")
            );
        ...
        if (isSysCall) {
            ...
            require(
                xheader.sourceChainId == nominaCChainId && xmsg_.sender == CChainSender
    ...
    function addValidatorSet(uint64 valSetId, XTypes.Validator[] calldata validators) external {
        require(msg.sender == address(this), "NominaPortal: only self");
        require(_xmsg.sourceChainId == nominaCChainId, "NominaPortal: only cchain");
        require(_xmsg.sender == CChainSender, "NominaPortal: only cchain sender");
    ...
    function _addValidatorSet(uint64 valSetId, XTypes.Validator[] calldata validators) internal {
        ...
        require(numVals > 0, "NominaPortal: no validators");
        require(valSetTotalPower[valSetId] == 0, "NominaPortal: duplicate val set");
        ...
        for (uint256 i = 0; i < numVals; i++) {
            ...
            require(val.addr != address(0), "NominaPortal: no zero validator");
            require(val.power > 0, "NominaPortal: no zero power");
            require(_valSet[val.addr] == 0, "NominaPortal: duplicate validator");
    ...
    * @notice Return true if all actions are paused
    ...
    function _setXMsgMinGasLimit(uint64 gasLimit) internal {
        require(gasLimit > 0, "NominaPortal: no zero min gas");
        require(gasLimit < xmsgMaxGasLimit, "NominaPortal: not below max");
    ...
    function _setXMsgMaxGasLimit(uint64 gasLimit) internal {
        require(gasLimit > xmsgMinGasLimit, "NominaPortal: not above min");
    ...
    function _setXMsgMaxDataSize(uint16 numBytes) internal {
        require(numBytes > 0, "NominaPortal: no zero max size");
    ...
    function _setXSubValsetCutoff(uint8 xsubValsetCutoff_) internal {
        require(xsubValsetCutoff_ > 0, "NominaPortal: no zero cutoff");
    ...
    function _setFeeOracle(address feeOracle_) internal {
        require(feeOracle_ != address(0), "NominaPortal: no zero feeOracle");
