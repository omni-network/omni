...
func TestDeploy(t *testing.T) {
	...
	commands := []string{"deploy", "upgrade"}
	for _, cmd := range commands {
		...
		err = os.MkdirAll(filepath.Join(def.Testnet.Dir), 0o755) //nolint: gocritic // this is for testing
		require.NoError(t, err)
		...
		for _, file := range files {
			t.Run(cmd+"_"+filepath.Base(file), func(t *testing.T) {
				t.Parallel()
