...
// FailedPermanentlyError is an error raised by Do when the
...
func Do[T any](ctx context.Context, maxAttempts int, strategy Strategy, op func() (T, error)) (T, error) {
	...
	var err error
	if maxAttempts < 1 {
		return empty, errors.New("need at least 1 attempt to run op, but have %d max attempts", maxAttempts)
	}
	...
	for i := 0; i < maxAttempts; i++ {
		...
		// Don't sleep when we are about to exit the loop & return FailedPermanentlyError
		if i != maxAttempts-1 {
			time.Sleep(strategy.Duration(i))
		}
	}

	return empty, &FailedPermanentlyError{
