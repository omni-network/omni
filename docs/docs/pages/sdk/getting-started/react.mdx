---
sidebar_position: 1
title: Getting Started using the React package
description: Guide to getting started with the Omni SDK using the React package.
---
...
Once your project meets the prerequisites, install our packages:

:::code-group
```bash [pnpm]
pnpm i @omni-network/react @omni-network/core
```

```bash [npm]
npm i @omni-network/react @omni-network/core
```

```bash [yarn]
yarn add @omni-network/react @omni-network/core
```

```bash [yarn]
bun i @omni-network/react @omni-network/core
```
:::

- `@omni-network/react` is our typescript react hooks.
- `@omni-network/core` is our core package, a typescript interface for using SolverNet.
