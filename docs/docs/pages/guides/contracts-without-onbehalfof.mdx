...
This guide demonstrates how to use handle contracts that do not support the `onBehalfOf` pattern. And instead, credit positions directly to `msg.sender`.

The solutions presented here only apply to contracts that credit tokenized, transferrable positions.

### 1:1 Deposits

For tokenized positions that mint 1:1 with deposit amount, you can use a simple multi-call order.
...
The address the receives vault tokens post-deposit is the `SolverNetExecutor` - the contract that executes order calls.
