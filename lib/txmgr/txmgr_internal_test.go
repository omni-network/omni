...
func (g *gasPricer) shouldMine(gasFeeCap *big.Int) bool {
	return bi.LTE(g.expGasFeeCap(), gasFeeCap)
}

func (g *gasPricer) feesForEpoch(epoch int64) (*big.Int, *big.Int) {
	epochBaseFee := bi.MulRaw(g.baseBaseFee, epoch)
	epochGasTipCap := bi.MulRaw(g.baseGasTipFee, epoch)
...
func (g *gasPricer) baseFee() *big.Int {
	return bi.MulRaw(g.baseBaseFee, g.getEpoch())
}
...
func (b *mockBackend) EstimateGas(ctx context.Context, msg ethereum.CallMsg) (uint64, error) {
	...
	if bi.LT(msg.GasFeeCap, msg.GasTipCap) {
...
func (b *mockBackend) TransactionReceipt(ctx context.Context, txHash common.Hash) (*types.Receipt, error) {
	...
	return &types.Receipt{
		...
		BlockNumber:       bi.N(txInfo.blockNumber),
	}, nil
}
...
func TestTxMgrBlocksOnFailingRpcCalls(t *testing.T) {
	...
	h.backend.setTxSender(sendTx)
	resTx, receipt, err := h.mgr.sendTx(t.Context(), orig)
	require.NoError(t, err)
	require.NotNil(t, receipt)
...
func TestTxMgr_CraftTx(t *testing.T) {
	...
	tx, err := h.mgr.craftTx(t.Context(), candidate)
	require.NoError(t, err)
	require.NotNil(t, tx)
	...
	require.Equal(t, startingNonce, tx.Nonce())
...
func TestTxMgr_EstimateGasFails(t *testing.T) {
	...
	_, err = h.mgr.craftTx(t.Context(), candidate)
...
func TestTxMgrDoesntAbortNonceTooLowAfterMiningTx(t *testing.T) {
	...
	sendTx := func(ctx context.Context, tx *types.Transaction) error {
		...
		case bi.LT(tx.GasFeeCap(), h.gasPricer.expGasFeeCap()):
			return nil
...
func doGasPriceIncrease(t *testing.T, txTipCap, txFeeCap, newTip,
	...
	tx := types.NewTx(&types.DynamicFeeTx{
		GasTipCap: bi.N(txTipCap),
		GasFeeCap: bi.N(txFeeCap),
	})
	newTx, err := mgr.increaseGasPrice(t.Context(), tx)
...
func testIncreaseGasPriceLimit(t *testing.T, lt gasPriceLimitTest) {
	...
	borkedBackend := failingBackend{
		gasTip:              bi.N(borkedTip),
		baseFee:             bi.N(borkedFee),
...
func TestMinFees(t *testing.T) {
	...
	for _, tt := range []struct {
		...
		},
		{
			...
			minBaseFee:       bi.N(10_000_000),
			expectMinBaseFee: true,
		},
		{
			desc:            "high-min-tipcap",
			minTipCap:       bi.N(1_000_000),
		...
		t.Run(tt.desc, func(t *testing.T) {
			...
			tip, baseFee, err := h.mgr.suggestGasPriceCaps(t.Context())
			require.NoError(t, err)
