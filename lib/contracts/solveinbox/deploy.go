...
// isDeployed returns true if the SolveInbox contract is already deployed to its expected address.
...
// DeployIfNeeded deploys a new SolveInbox contract if it is not already deployed.
...
// Deploy deploys a new SolveInbox contract and returns the address and receipt.
func Deploy(ctx context.Context, network netconf.ID, backend *ethbackend.Backend) (common.Address, *ethtypes.Receipt, error) {
	...
	cfg := DeploymentConfig{
		...
		Portal:          addrs.Portal,
		Outbox:          addrs.SolveOutbox,
		ExpectedAddr:    addrs.SolveInbox,
	}
...
func deploy(ctx context.Context, cfg DeploymentConfig, backend *ethbackend.Backend) (common.Address, *ethtypes.Receipt, error) {
	...
	impl, tx, _, err := bindings.DeploySolveInbox(txOpts, backend)
	if err != nil {
...
func packInitCode(cfg DeploymentConfig, impl common.Address) ([]byte, error) {
	inboxAbi, err := bindings.SolveInboxMetaData.GetAbi()
	if err != nil {
