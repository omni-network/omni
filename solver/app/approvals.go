...
func approveOutbox(ctx context.Context, chain netconf.Chain, backend *ethbackend.Backend, solverAddr, outboxAddr common.Address) error {
	...
	for _, token := range tokens.ByChain(chain.ID) {
		if token.IsNative() {
			continue
		}
...
func approveToken(ctx context.Context, backend *ethbackend.Backend, token tokens.Token, solverAddr, outboxAddr common.Address) error {
