...
var claimants = map[tokens.Asset]map[netconf.ID]common.Address{
...
func Claimant(network netconf.ID, token tokens.Asset) (common.Address, bool) {
...
func getClaimant(network netconf.ID, order Order) (common.Address, bool, error) {
	...
	for _, output := range minReceived {
		cs = append(cs, claimants[output.Token.Asset][network])
	}
